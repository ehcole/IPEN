import h5py
import numpy as np
#TODO: replace with paths to .h5 files
#f0 should be the case with all plates removed (-7.h5)
f0 = h5py.File("/scratch/wrm_fluxoe/ehcole/IPEN_Analysis/version3/IPEN-015/4-3-7.h5", 'r')
...
for i in range(1, 8):
    try:
        f = h5py.File("/scratch/wrm_fluxoe/ehcole/IPEN_Analysis/version3/IPEN-015/4-3-" + str(i) + ".h5", 'r')
    except:
        exit(0)
    else:
        try:
